...
Инструмент рассчитан на длительную разработку людьми и ИИ. Текущее принятое состояние собственной политики и архитектуры публикуется в [Обсерватории политики](https://netkeep80.github.io/repo-guard/).
...
Для файлов, которые должны изменяться только вне `PR`-транзакции, доверенная базовая политика может задать `paths.pr_immutable`. Любое изменение совпавшего пути через `PR`, включая удаление и переименование, блокируется; изменить сам набор через `PR` тоже нельзя, и `GovernanceGrant` этот запрет не отменяет. Первичное включение ограничения выполняется вне `check-pr`, например прямым коммитом владельца в базовую ветку.

## Установка и закрепление версии

Требуется `Node.js` 20+. До публикации выпуска закрепляйтесь за полным SHA принятого коммита; после публикации — за соответствующим неизменяемым тегом `vX.Y.Z`. Состояние версии, тега и выпуска показывает [Обсерватория политики](https://netkeep80.github.io/repo-guard/), процесс выпуска описан в [`RELEASING.md`](RELEASING.md).

Для проверки в репозитории-потребителе закрепляйте `GitHub Action` за полным принятым `SHA`:

```yaml
- uses: netkeep80/repo-guard@<40-char-SHA>
```

Для локальной проверки используйте точную рабочую копию того же принятого коммита:
...
Сгенерированный workflow закрепляет действие за переданным коммитом или тегом. Официальная ссылка выпуска проверяется через `npm run verify:release-ref`.
...
Машинная схема — источник истины для структуры, типов и перечислений языка политики: [`schemas/repo-policy.schema.json`](schemas/repo-policy.schema.json). Исполняемые примеры находятся в [`examples/scenarios/`](examples/scenarios/); README не дублирует полный справочник правил.
...
`scope` — исполняемая граница; `must_touch` требует заявленное изменение, `must_not_touch` запрещает совпавшие пути.

`repo-policy.json` задаёт доверенные потолки `diff_rules.max_new_docs`, `max_new_files` и `max_net_added_lines`. Одноимённые `ChangeIntent.budgets` могут только ужесточить их: при двух значениях действует `min(policy, intent)`. Нулевой потолок не расширяется; профильные лимиты остаются независимыми veto.
...
Репозиторий применяет собственную политику к себе. Для внешнего репозитория начальная конфигурация создаётся командой `repo-guard init --action-ref <ref>` из проверенного экземпляра; сгенерированный workflow проверяется [`tests/test-init.mjs`](tests/test-init.mjs).
