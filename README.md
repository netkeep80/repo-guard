...
Требуется `Node.js` 20 или новее. Текущее состояние пакета имеет версию `3.0.0`, но официальный `v3.0.0` ещё не опубликован. До завершения проверки выпуска непинованные вызовы `npm` и `npx` не считаются способом получить текущий v3-кандидат; полный процесс выпуска описан в [`RELEASING.md`](RELEASING.md).
