...
- сопровождается trusted authorization. На текущий момент `repo-guard`
  автоматически распознаёт два источника:
  - автор связанного issue имеет права `write`, `maintain` или `admin`
    (либо `author_association` входит в `OWNER`, `MEMBER`, `COLLABORATOR`);
  - на issue или PR выставлен label `governance-approved` мейнтейнером.

  Сигналы `codeowner_approved` и `trusted_team_approval` поддерживаются
  движком как валидные источники доверия, но **пока не резолвятся
  автоматически из GitHub API** — это запланированная работа. До её
  завершения `CODEOWNERS`-одобрение или одобрение от trusted team нужно
  материализовать через label `governance-approved` (или явное доверие
  автора issue), иначе governance-PR будет заблокирован.
