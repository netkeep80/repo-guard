...
Для файлов, которые должны изменяться только вне `PR`-транзакции, доверенная базовая политика может задать `paths.pr_immutable`. Любое изменение совпавшего пути через `PR`, включая удаление и переименование, блокируется; изменить сам набор через `PR` тоже нельзя, и `GovernanceGrant` этот запрет не отменяет. Первичное включение такого ограничения выполняется вне `check-pr`, например прямым коммитом владельца в базовую ветку.

## Установка и закрепление версии

Требуется `Node.js` 20 или новее. Версия текущей рабочей копии определяется `package.json`, но сам номер версии не доказывает, что соответствующий выпуск уже опубликован. Фактическое состояние версии, тега и выпуска показывает [Обсерватория политики](https://netkeep80.github.io/repo-guard/); полный процесс выпуска описан в [`RELEASING.md`](RELEASING.md). До публикации выпуска закрепляйтесь за полным 40-символьным SHA принятого коммита; после публикации можно использовать соответствующий неизменяемый тег `vX.Y.Z`.
...
`diff_rules.max_new_docs`, `diff_rules.max_new_files` и `diff_rules.max_net_added_lines` из доверенной `repo-policy.json` являются потолками репозитория. Одноимённые `ChangeIntent.budgets` — только дополнительное сужение для конкретного изменения: если заданы оба значения, эффективный предел равен `min(policy, intent)`. Поэтому PR может запросить более строгий бюджет, но не может расширить доверенный потолок; нулевой потолок также не расширяется. Независимые ограничения профиля изменения и другие правила остаются дополнительными veto.
