...
Исходный измеримый срез и предыдущие принятые этапы зафиксированы в `docs/architecture-compression-3-baseline.md`. Этот документ продолжает baseline и не переписывает замороженные исходные значения C3.0.
...
Он возвращает нормализованное множество строковых значений якоря. При дедупликации значения не теряются исходные местоположения: provenance сохраняет все экземпляры, включая несколько одинаковых значений в разных файлах или позициях.
...
Здесь семантика наличия доказательства та же, а источник триггера меняется с diff на канонический факт `ChangeIntent`.
...
Отдельный evaluator покрытия значений не требуется. Повторные местоположения целевых якорей сохраняются в provenance правого факта и проходят через обычный результат отношения.
...
Историческая семантика `must_resolve` требовала проверки перехода. Поэтому скомпилированный экземпляр отношения несёт локальное переопределение:
...
Из рабочего union удалены два вида ограничений:
...
Удаление защищено отдельным тестом: он проверяет отсутствие обоих файлов и отсутствие вызовов старых функций в `constraint-program`, runtime constraints и anchor reporting.
...
Таким образом, reporting не становится скрытым вторым evaluator.
...
Компилятор высокоуровневого правила не дублирует публичное имя `set_subset`. Он запрашивает общую семантику сравнения множеств `left_subset`, а конкретный kind разрешается через metadata единственного descriptor registry.
...
## 9. Проверка эквивалентности и TDD

Работа шла через отрицательные проверочные состояния.

Начальные falsifier-запуски #1177 и #1178 доказали отсутствие требуемых источников фактов, понижений и provenance до production-реализации.

После реализации отдельный deletion falsifier #1206 имел ровно две причины отказа:
...
Все проверки отсутствия старых вызовов в активных consumers на том же запуске уже проходили. После физического удаления обоих файлов полный запуск #1208 на head:
...
Отдельный запуск #1205 до удаления старого файла подтвердил phase override и полный набор существующих тестов.
...
Они остаются отдельными рабочими видами и должны рассматриваться следующим ограниченным этапом #374 только после отдельного falsifier и доказательства эквивалентности.
