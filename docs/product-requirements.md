...
Целевая фазовая семантика: черновик допускает явно обозначенную неполноту; готовый PR означает, что его точное состояние предъявлено для приёмки; принятый коммит требует собственных свидетельств после слияния. Это методика, не обещание автоматического переключения всех уровней проверки. В собственном репозитории обычная проверка `PR` и `trusted-enforcement` пропускают черновики; изменения тела готового PR переоцениваются событием `edited`. Изменение тела связанной задачи сначала переводит прежний `trusted-enforcement` в `pending`; если затронут ровно один открытый готовый PR против `main`, принятый оценщик автоматически переоценивает тот же неизменный `HEAD` и публикует свежий успех или отказ. При нескольких затронутых PR автоматического веерного запуска нет: они остаются `pending` до обычного события `PR`. Дрейф базы делегирован настроенной защите ветки, которая делает PR `behind`. Генерируемый `init` потребительский рабочий процесс этого полного доверенного контура не обещает.
...
- В собственном доверенном контуре изменение тела PR запускает новую оценку. Изменение тела связанной задачи консервативно инвалидирует старый `dedicated status` в `pending`; для ровно одного затронутого открытого готового PR принятый `check-pr` автоматически переоценивает тот же `HEAD` и публикует новый `GREEN/RED`. Если затронуто несколько PR, автоматического веерного запуска нет и они остаются `pending`. Дрейф `main` не обслуживается планировщиком: при включённом требовании актуальной ветки платформа переводит PR в `behind`.
- `init` создаёт четыре файла и по умолчанию выбирает `blocking`. Его потребительский рабочий процесс обрабатывает `opened`, `edited`, `synchronize`, `reopened`, `ready_for_review`, поэтому изменение только тела PR переоценивается без нового коммита. Базовый рабочий процесс не создаёт `dedicated trusted producer`, не обрабатывает `issues: edited` и не меняет `branch protection`. Полный контур подключается отдельно по [руководству](trusted-deployment.md).
