...
- В собственном доверенном контуре изменение тела PR запускает новую оценку. Изменение тела связанной задачи консервативно инвалидирует старый `dedicated status` в `pending`; для ровно одного затронутого открытого готового PR принятый `check-pr` автоматически переоценивает тот же HEAD и публикует новый `GREEN/RED`. Если затронуто несколько PR, автоматического fan-out нет и они остаются `pending`. Дрейф `main` не обслуживается scheduler'ом: при включённом требовании актуальной ветки платформа переводит PR в `behind`.
- `init` создаёт четыре файла и по умолчанию выбирает `blocking`. Его потребительский workflow обрабатывает `opened`, `edited`, `synchronize`, `reopened`, `ready_for_review`, поэтому изменение только тела PR переоценивается без нового коммита. Базовый workflow не создаёт `dedicated trusted producer`, не обрабатывает `issues: edited` и не меняет `branch protection`. Полный контур подключается отдельно по [руководству](trusted-deployment.md).
