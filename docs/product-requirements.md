...
Средства: защищённые документы жизненного цикла, отношения документов, отдельная санкция перехода, точные ссылки. `contract-conformance` проверяет настроенные структурные условия, но сам не выдаёт полномочие принять контракт. В `anum_docs` принятый `v0.11` сосуществует с неполным кандидатом `v0.12`.
...
Средства: `scope`, `must_not_touch`, потолки политики, governance, `pr_immutable`; для постоянной границы нужна доверенная политика, поскольку сам автор может переписать своё намерение. В `anum_parser` независимость семантического и визуального пакетов дополнительно проверяется точными зависимостями и предметным CI. Известный пробел прежнего пути в обычном намерении указан ниже.
...
Средства: явный `advisory`, начальные настройки, `doctor`, короткие примеры. Текущие значения по умолчанию и охват `doctor` ещё не полностью удовлетворяют желаемому удобству: это задача развития, а не скрытое изменение поведения данным документом.
...
| `ChangeIntent.scope`, `must_not_touch` | `CORE`, `RG-03` | Сохранить смысл границы; устранить доказанный пробел переименования |
...
Целевая фазовая семантика: черновик допускает явно обозначенную неполноту; готовый PR означает, что его точное состояние предъявлено для приёмки; принятый коммит требует собственных свидетельств после слияния. Это методика, не обещание уже реализованного автоматического переключения уровней проверок. В собственном репозитории реальная проверка PR и `trusted-enforcement` пропускают черновики, которые платформа не разрешает сливать. Начальный потребительский процесс ведёт себя иначе. Для готового PR нельзя полагаться на отсутствующую проверку или старый результат.

## Границы текущей реализации

Срез аудита: `04c44b3ac7cd5e7e8dc3392144a774f53c5c22b4`, пакет `3.1.0`. Нормативное требование не означает, что каждый потребитель уже настроен в соответствии с ним.

- `scope` и `must_not_touch` сейчас смотрят на новый путь переименованного файла. Локальный контрпример переноса `private/authority.md` в `public/authority.md` проходит намерение `scope: public/**`, `must_not_touch: private/**`. Защита governance и `pr_immutable` учитывает оба пути. Коррекция существующего сведения правил отслеживается в [#568](https://github.com/netkeep80/repo-guard/issues/568); новый примитив не нужен.
- Источник санкции — текущее тело связанной задачи и проверяемые права её автора. Это не подписанная неизменяемая санкция на конкретный `HEAD`, не срок действия и не проверка личности каждого редактора тела. Ограничивайте доступ к доверенному источнику. Временную санкцию убирают после её применения; история сохраняет свидетельство.
- Триггеры собственных проверок не охватывают каждое изменение тела PR, задачи и базовой ветки. Это доказанный пробел повторного наблюдения, но обход слияния устаревшим успехом требует отдельного воспроизводимого эксперимента с настройками платформы. Перед приёмкой нужна свежая проверка входов; один неизменившийся `HEAD` не доказывает неизменность внешней санкции.
- `init` создаёт четыре файла и по умолчанию выбирает `blocking`; `doctor` не проверяет обязательные статусы и права их производителя. Рекомендация начинать с `advisory` не меняет эти значения автоматически; минимальный следующий шаг исследуется в [#569](https://github.com/netkeep80/repo-guard/issues/569).
- Отношения и пакеты подтверждают только настроенные предикаты; они не исполняют произвольную предметную проверку. В аудированных `aprover` и `anum_parser` точные зависимости проверяет обычный CI; защищённый барьер слияния на `main` не был настроен. Нельзя выдавать эти репозитории за полное доказательство независимого запрета обхода.

Исправления и повторные проверки ведутся в [#518](https://github.com/netkeep80/repo-guard/issues/518) и его действующих дочерних задачах. Этот список фиксирует пределы исходной редакции требований; текущий статус реализации определяется точным кодом и свидетельствами, а не стиранием исторического контрпримера.
