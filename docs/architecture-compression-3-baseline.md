...
## 13. C3.2 — структурное сжатие и чистое макроразворачивание

C3.2 заменяет специальный полный ориентированный граф, который раньше генерировался из `contract_conformance.cochange`, на одну универсальную группу совместного изменения:
...
Группа имеет семантический ключ `id`. Порядок участников не влияет на смысл. Добавление группы является усилением, удаление — ослаблением, изменение состава существующей группы считается несопоставимым изменением. Канонический указатель имеет форму:

```text
/cochange_groups/<id>
```

Направленные `cochange_rules` остаются отдельным универсальным отношением и не являются совместимым псевдонимом группы.

### 13.1 Чистота пакета

`contract_conformance` понижает своё поле `cochange` ровно в одну группу с идентификатором:

```text
contract-conformance
```

После разворачивания исходная лексика ролей не попадает в каноническое ядро. `Constraint Program`, компилятор политики, ядро отношений и рабочий вычислитель не знают:
...
Удалены обратное распознавание сгенерированных рёбер, специальная модель рёбер ролей и позиционная идентичность макроса. Высокоуровневый пакет остаётся только источником чистого понижения в общие ограничения.

### 13.2 Машинные инварианты

На проверенном состоянии PR #389 до его принятия:

```text
head = 8979b866dc7248313f6e330b148f58bcb0d1b855
Actions run = 34139941120
validate = success
smoke-pack = success
test files = 77 passed, 0 failed
```

Измеритель фиксирует:
...
Тем самым C3.2 выполняет цель C3.0 для высокоуровневой возможности:
...
## 14. C3.3a — понижение исторической семантики путей, бюджетов и совместных изменений

C3.3a (#390) — первый ограниченный срез C3.3. Он не вводит новый публичный язык правил: существующая семантика путей, обязательных изменений, бюджетов и совместных изменений компилируется в то же рабочее представление `primitive_relation`, установленное C3.1.

Шесть исторических видов рабочих ограничений удалены из производственных исходников:
...
Вместо них используются три малых внутренних дескриптора отношений:
...
Они принадлежат единственному реестру `relation-kernel` и имеют `public = false`. Поэтому публичная схема отношений не расширена.

### 14.1 `FactRef` для фактов изменений
...
Для источника `diff` после дополнительного сжатия осталось ровно два вида селекторов:
...
Поэтому семантика выражается без отдельного производного вида факта:
...
При понижении не восстановлены исторические диагностические адаптеры вроде:
...
`numeric_bound` сохраняет исходный операнд и выбранные `source_values`, а `set_presence_implies` сохраняет канонические операнды и прочитанные факты левой и правой стороны. Поэтому нарушающие ограничение пути и требуемый селектор доступны структурированным отчётам непосредственно из доказательств отношения.
...
Проверка политики PR на этом запуске была пропущена только потому, что PR оставался черновиком. После синхронизации документации PR должен пройти ту же самопроверку уже после перевода в состояние готовности к рецензированию.
...
Таким образом, ограниченный срез уменьшает историческую рабочую поверхность, сохраняя закрытое при ошибке поведение и доказательные данные. Следующие семейства C3.3 могут понижаться тем же методом только при отдельном доказательстве эквивалентности; этот результат не является разрешением заранее расширять алгебру отношений.