...
Отдельно самопроверка применяет правило `markdown_language` из собственной политики ко всем документным поверхностям `Markdown`: корневым документам, `docs/`, `templates/`, `examples/` и Markdown-файлам `.github/`. Тестовые фикстуры намеренно остаются вне этой области, поскольку они моделируют входные данные внешних репозиториев, а не документацию `repo-guard`.
