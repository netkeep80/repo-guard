...
Подход отклонён: он сохраняет сложность специально ради демонстрации и противоречит программе архитектурного сжатия 3.0.
...
Настройка защиты ветки и набор обязательных проверок не копируются в новый файл политики. Их точное состояние проверяется как внешнее доказательство приёмки `GitHub` при приёмке PR, а не превращается в семантику исполнения.
...
## 10. Универсальное уточнение строгости перед переписыванием политики

Текущий `compareConstraintPrograms()` отдельно сравнивает известные ограничения строгости, а затем строит `unknownProjection()` для оставшихся частей политики, не представленных в программе ограничений.
...
Алгоритм не должен содержать имена этих полей. Он должен универсально работать с любыми верхнеуровневыми ключами остаточной проекции.

Вложенный объект внутри одного неизвестного верхнеуровневого ключа остаётся одной единицей сравнения. C3.4 не вводит рекурсивный произвольный язык сравнения `JSON` и не усложняет сравнение строгости больше, чем требуется для узкой доверенной санкции.

Новый вид исполнения или дескриптор отношения для этого не нужен.

## 11. Правдивость покрытия самоприменения
...
Не требуется перегружать существующий тест `doctor` другой ответственностью. Допустим отдельный сфокусированный фальсификатор живого примера, если так границы тестов будут проще.
...
Если возможность продукта намеренно не применяется к самому `repo-guard`, её специализированный тест доказывает эту возможность, а файл исключений объясняет, почему самоприменение было бы искусственным.
...
После слияния не остаётся слоя совместимости между старой и новой собственной политикой.

## 14. TDD и приёмка

Каждый реализационный срез выполняется через тестовый `RED` перед изменением рабочего кода.
