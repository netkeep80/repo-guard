...
В полном зелёном запуске `34121685027` проверки `test-canonical-relation-kernel.mjs`, `test-document-facts-boundary.mjs` и `test-transition-rules.mjs` проходят через новую форму исполнения.
...
Полный черновой запуск `34121685027` на голове `996e8ce234572a15e45c68c53fd4fe44ed7e48d6` завершён успешно: новый тест ядра, переходные правила, междокументная трассируемость, продвижение контрактов и остальные обнаруженные тесты зелёные.
...
- [x] **Шаг 1: добавить проверку согласованности схемы и ядра**

Тест извлекает `kind.const` из `definitions.document_relation_rule.oneOf` и требует точного равенства с `relationDescriptors()`. Это доказано полным зелёным запуском `34121685027`.

- [x] **Шаг 2: расширить метрики сжатия**

Машинно зафиксировано:
...
Также восстановлен корректный учёт шести типов документных фактов после перехода на `FactRef`.

- [x] **Шаг 3: обновить канонический архитектурный отчёт**

`docs/architecture-compression-3-baseline.md` сохраняет неизменным исходный срез C3.0 и содержит отдельный срез C3.1 с точной головой, машинными показателями и воротами на #373. Публичный синтаксис политики не изменён; старые внутренние селекторы и отдельные виды времени исполнения удалены, а не оставлены псевдонимами.
...
Изменённые файлы `dist` приведены к точному выводу `TypeScript`.

- [x] **Шаг 2: проверить свежесть генерации**

На голове `996e8ce234572a15e45c68c53fd4fe44ed7e48d6` в запуске `34121685027` шаг `Verify generated dist freshness` завершён успешно.

- [x] **Шаг 3: получить полностью зелёный набор тестов**

На той же голове полный обнаруженный набор тестов завершён успешно; `validate` и `smoke-pack` зелёные.
...
Черновой запуск уже зелёный, но шаг самопроверки `check-pr` в нём ожидаемо пропущен. После перевода PR из черновика новый запуск обязан реально выполнить этот шаг успешно.
