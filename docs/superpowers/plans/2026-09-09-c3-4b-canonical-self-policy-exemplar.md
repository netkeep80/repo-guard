...
Открыть черновой `PR` с тестовым состоянием `RED`.
...
Зафиксировать точные причины `RED` в контексте `PR` и задачи.
...
- исправить комментарий: общий инвентарь проверяет новый тест C3.4b, а `tests/test-self-hosting.mjs` остаётся тестом диагностики;
...
- больше не утверждать, что тест диагностики строит матрицу возможностей;
- описать два независимых слоя доказательства:
  1. реальное самоприменение через `CI`, `Action` и политику;
  2. выводимый инвентарь реестров и ссылочная целостность исключений через тест C3.4b;
...
- в разделе самоприменения сослаться на реальную компактную собственную политику и четыре реальные документные связи;
- не добавлять `workflow DSL` и не дублировать `CI` как ручной манифест.

## Шаг 5. GREEN и архитектурная проверка

После единого коммита реализации запустить полный `CI` в черновом `PR`.
...
Если удобнее для доказуемости, документацию и шаблоны можно вынести в следующий отдельный коммит, но все изменения остаются в одном `PR` и каждый промежуточный `head` должен иметь понятную причину состояния.

## Шаг 6. Приёмка готового PR

До перевода в готовое состояние провести ревью полного `diff` по #423 и принятому дизайну.

Перевести `PR` в готовое состояние только после зелёного чернового `CI`.
...
Готовый `PR` должен сам доказать атомарную управляющую миграцию через доверенный `grant` #423.

Нельзя обходить падение `policy check` расширением `grant` за пределы трёх разрешённых указателей политики без отдельного доказанного изменения дизайна.

## Шаг 7. Слияние и проверка после слияния
...
Слить точный `head` обычным `merge`.
