...
export interface RepositoryFactsPolicyProjection extends AnchorPolicyProjection {
  paths: { operational_paths?: readonly string[] | null; pr_immutable?: readonly string[] | null };
...
export const listTrackedFiles = (repoRoot: string): string[] => execFileSync("git", ["ls-files", "-z"], { encoding: "utf-8", cwd: repoRoot }).split("\0").filter(Boolean);
