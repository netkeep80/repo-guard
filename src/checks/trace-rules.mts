...
export function checkTraceRuleResult(result: { id: string; [key: string]: unknown }) {
  if ((result.kind as string) === "must_resolve") return checkMustResolve(result as unknown as MustResolveDiagnostic);
  if (["changed_files_require_evidence", "declared_anchors_require_evidence"].includes(result.kind as string)) return checkEvidence(result as unknown as EvidenceDiagnostic);
