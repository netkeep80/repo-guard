...
function factRef(relation: PrimitiveRelation, role: string): FactRef {
  ...
  if (!operand || !("source" in operand)) throw new Error(`relation "${relation.relation_id}" requires fact operand "${role}"`);
  return operand as FactRef;
}

function documentTarget(relation: PrimitiveRelation, role: string): RelationDocumentTarget {
  const operand = relation.operands[role];
  if (!operand || "source" in operand) throw new Error(`relation "${relation.relation_id}" requires document operand "${role}"`);
  return operand as RelationDocumentTarget;
}

function documentFactSelector(ref: FactRef, relationId: string): Extract<FactRef, { source: "document" }>["selector"] {
  if (ref.source !== "document") throw new Error(`relation "${relationId}" requires document fact operands`);
  return ref.selector;
}

function factOperand(facts: RelationEvaluationFacts, relation: PrimitiveRelation, role: string) {
...
function scalarStrictlyGreater(facts: RelationEvaluationFacts, relation: PrimitiveRelation) {
  ...
  if (!baseRef || !headRef || !baseSelector || !headSelector) return {
    ok: false,
  ...
  if (baseSelector.path !== headSelector.path) return {
    ok: false,
  ...
  if (!base.ok) return { ok: false, message: `document relation "${relation.relation_id}" could not read BASE: ${base.error.message}`, ...diagnostic, data: { base, head } };
  if (!head.ok) return { ok: false, message: `document relation "${relation.relation_id}" could not read HEAD: ${head.error.message}`, ...diagnostic, data: { base, head } };
  if (comparator !== "semver") return { ok: false, message: `document relation "${relation.relation_id}" has unsupported comparator`, ...diagnostic, data: { base, head } };
  if (typeof base.value !== "string" || typeof head.value !== "string") return { ok: false, message: `document relation "${relation.relation_id}" requires string operands`, ...diagnostic, data: { base, head } };
  const baseTuple = parseSemverCore(base.value), headTuple = parseSemverCore(head.value);
  if (!baseTuple) return { ok: false, message: `document relation "${relation.relation_id}" has malformed BASE semver`, ...diagnostic, data: { base, head } };
  if (!headTuple) return { ok: false, message: `document relation "${relation.relation_id}" has malformed HEAD semver`, ...diagnostic, data: { base, head } };
...
function scalarEqual(facts: RelationEvaluationFacts, relation: PrimitiveRelation) {
  ...
  if (!left.ok || !right.ok) return { ok: false, message: `document relation "${relation.relation_id}" could not read scalar operands`, data };
...
function scalarEqualsLiteral(facts: RelationEvaluationFacts, relation: PrimitiveRelation) {
  ...
  if (!source.ok) return { ok: false, message: `document relation "${relation.relation_id}" could not read scalar operand`, data };
...
function setAllOrNone(facts: RelationEvaluationFacts, relation: PrimitiveRelation) {
  const source = factOperand(facts, relation, "source");
  const universe = relation.parameters.universe;
  const data = { kind: relation.primitive, source, universe };
  if (!source.ok) return { ok: false, message: `relation "${relation.relation_id}" could not read selected set`, data };
  ...
  const selected = source.value as string[];
  const normalizedUniverse = [...new Set(universe as string[])].sort();
...
function referencedPathsExist(facts: RelationEvaluationFacts, relation: PrimitiveRelation) {
  const source = factOperand(facts, relation, "source");
  if (!source.ok) return { ok: false, message: `document relation "${relation.relation_id}" could not read repository path references`, data: { kind: relation.primitive, source } };
  if (!Array.isArray(source.value)) return { ok: false, message: `document relation "${relation.relation_id}" did not produce a repository path set`, data: { kind: relation.primitive, source } };
...
function setRelation(facts: RelationEvaluationFacts, relation: PrimitiveRelation, comparison: "equal" | "left_subset") {
  ...
  if (!Array.isArray(left.value) || !Array.isArray(right.value)) return { ok: false, message: `document relation "${relation.relation_id}" did not produce string sets`, data: { kind: relation.primitive, left, right } };
...
function readReferencedPointerTarget(facts: RelationEvaluationFacts, target: RelationDocumentTarget, pointer: string) {
...
function referencedPointerExists(facts: RelationEvaluationFacts, relation: PrimitiveRelation) {
  const source = factOperand(facts, relation, "source");
  if (!source.ok || typeof source.value !== "string") return { ok: false, message: `document relation "${relation.relation_id}" could not read JSON Pointer source`, data: { kind: relation.primitive, source } };
...
const DESCRIPTORS: readonly RelationDescriptor[] = [
  {
...
export function relationDescriptors(): readonly RelationDescriptor[] {
  return DESCRIPTORS;
}

export function relationDescriptor(kind: string): RelationDescriptor {
