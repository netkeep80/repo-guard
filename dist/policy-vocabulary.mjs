...
function resolveLocalRef(rootSchema, rawSchema) {
    let schema = object(rawSchema);
    const seen = new Set();
    while (typeof schema.$ref === "string" && schema.$ref.startsWith("#/")) {
        const ref = schema.$ref;
        if (seen.has(ref))
            throw new Error(`circular local schema reference: ${ref}`);
        seen.add(ref);
        let value = rootSchema;
        for (const token of ref.slice(2).split("/"))
            value = object(value)[decodePointerToken(token)];
        schema = object(value);
    }
    return schema;
}
function discriminatedVariant(rootSchema, rawSchema, value) {
    ...
    const variants = Array.isArray(schema.oneOf)
        ...
        : [];
    if (!variants.length || !value || typeof value !== "object" || Array.isArray(value))
        return null;
    const variantConstants = variants.map((variant) => Object.entries(object(variant.properties))
        ...
        .map(([key, property]) => [key, object(property).const]));
    const candidateKeys = variantConstants[0]?.map(([key]) => key) ?? [];
    for (const key of candidateKeys) {
        if (!variantConstants.every((entries) => entries.some(([candidate]) => candidate === key)))
            continue;
        const constants = variantConstants.map((entries) => entries.find(([candidate]) => candidate === key)[1]);
        if (new Set(constants.map((item) => JSON.stringify(item))).size !== variants.length)
            continue;
...
function projectValue(rootSchema, rawSchema, value) {
    const schema = resolveLocalRef(rootSchema, rawSchema);
    const variant = discriminatedVariant(rootSchema, schema, value);
    if (variant === OMIT)
        return OMIT;
    if (variant)
        return projectValue(rootSchema, variant, value);
 ...
 * constraints.
 */
export function projectPolicyToCurrentVocabulary(rawPolicy, rawCurrentSchema) {
