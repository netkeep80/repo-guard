...
function checkPolicyDiscovery(repoRoot, packageRoot) {
    return check("repo-policy.json", () => {
        ...
        return { name: "repo-policy.json", status: PASS, message: `Valid (${effectivePolicy.repository_kind}, format ${effectivePolicy.policy_format_version}; enforcement ${String(effectivePolicy.enforcement?.mode || "blocking")})` };
    });
}
function githubRepositorySlug(repoRoot) {
    const workspace = process.env.GITHUB_WORKSPACE;
    const repository = process.env.GITHUB_REPOSITORY;
    if (workspace && resolve(workspace) === resolve(repoRoot) && repository && /^[^/]+\/[^/]+$/.test(repository))
        return repository;
    try {
...
function requiredStatusChecks(branch) {
    ...
    if (Array.isArray(required?.contexts))
        for (const context of required.contexts) {
            if (typeof context === "string" && context && !values.some((value) => value === context || value.startsWith(`${context}@app:`)))
                values.push(context);
        }
    return [...new Set(values)].sort();
}
function checkMergeBarrierCoverage(repoRoot) {
