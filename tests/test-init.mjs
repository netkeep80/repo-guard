...
describe("repo-guard init", () => {
  it("creates a valid default scaffold", () => {
    ...
    for (const path of ["repo-policy.json", ".github/workflows/repo-guard.yml", ".github/PULL_REQUEST_TEMPLATE.md", ".github/ISSUE_TEMPLATE/change-contract.yml"]) {
  ...
  it("generates pinned workflow and separate intent/grant templates", () => {
    ...
    assert.match(pr, /ChangeIntent/); assert.match(pr, /repo-guard-yaml/); assert.doesNotMatch(pr, /repo-guard-grant/);
  ...
  it("does not overwrite existing files", () => {
    ...
    assert.equal(run(["--repo-root", dir, "init"]).status, 0);
