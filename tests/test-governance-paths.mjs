...
describe("GovernanceGrant authorization", () => {
  ...
  it("allows exact or glob grant from trusted repository permission authority", () => {
  ...
  it("does not accept retired label/codeowner/team boolean shapes as authority", () => {
    for (const source of [{ governance_approved_label: true }, { codeowner_approved: true }, { trusted_team_approval: true }, { issue_author_permission_trusted: true }]) {
      assert.equal(checkGovernanceChangeAuthorization({ files: [file("repo-policy.json")], governancePaths: PATHS, governanceGrant: grant("repo-policy.json"), trustedAuthorizer: source }).ok, false);
    }
  });
